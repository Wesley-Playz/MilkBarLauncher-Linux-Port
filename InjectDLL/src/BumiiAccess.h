...
namespace DataTypes
{
    ...
    public:
        ...
        void WriteMiiData(int specificHeader = 0, int specificParameter = 0)
        {
            ...
            {
                std::cerr << "Bumii service not started... Cancelling bumii setup." << std::endl;
                return;
            }
            ...
            if (iterator > 1000)
            {
                std::cerr << "Could not find bumii headers." << std::endl;
                return;
            }

            uint64_t StartOfHeaders = bumiiFileStartAddr + (iterator * 4);

            std::cout << "Analyzing bumii headers..." << std::endl;
                ...
                for (int j = 0; j < childNum; j++)
                {
                    ...
                    if (mappings[header].count(param))
                    {
                        ...
                        if (param != 829051577) // For now, we will ignore Vec3
                        {
                            ...
                            memcpy(reinterpret_cast<void*>(childHeaderAddr + paramOffset), &val, 4);
        ...
        {
            int val;
            memcpy(&val, reinterpret_cast<void*>(addr + 2), 1);
            memcpy(reinterpret_cast<uint8_t*>(&val) + 1, reinterpret_cast<void*>(addr + 1), 1);
            memcpy(reinterpret_cast<uint8_t*>(&val) + 2, reinterpret_cast<void*>(addr), 1);
            return val;
        }
